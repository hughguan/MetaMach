...
MetaMach is a **specification-first repository with a working Rust implementation**. Version 0.6.0 spans M0–M4 + the 0.3.0 de-containerization consensus + the 0.4.0 gateway/ecosystem delta + ADR-029 project-based templates + ADR-031 DSL unification + ADR-032 Studio. 193 tests, CI-green. Current layout:
...
│   └── tests/           #   9 integration test files (193 tests total)
...
- Unit tests in `#[cfg(test)]` modules alongside source; integration tests in `janus/tests/` (9 files, 193 tests total).
