...
This is an **implemented Rust workspace plus specs** - not documentation-only. Milestones M0–M3 and M5 plus the 0.3.0 and 0.4.0 rearchitectures are built. **M4 is partial**: Task 4.2 (Offboard) + Task 4.3 (Onboard) are implemented and tested, Task 4.1's **SQLite Log Replay is implemented** (`AbsurdDb::replay_fallback` drains the ring buffer into the routed per-blueprint `metamach_step_meta` overlay on PG recovery), the **workflow-execution engine is implemented** (Phase 0a absurd adapter `init_absurd_schema`/`AbsurdPgAdapter`/`DurableEngine` + Phase 0b `workflow::run_workflow` - `Request::Dispatch` Contract 3.11 dispatches a blueprint's workflow onto absurd's pull-mode queue and drives real tmux steps under `janush` with exit-code capture, per-step checkpoints, lease renewal, and `tmux_alive` wired; `utc_03_01b` green), **cold-start re-exec + the retry loop are implemented** (Phase 1: `run_workflow` is a retry-claim loop that re-claims absurd's retry runs + resumes from the last `COMPLETED` checkpoint; `coldstart::reconcile` spawns `run_workflow` per non-terminal task instead of just logging; `max_attempts: 3`; `utc_03_03` green), and **the HITL resume loop is wired** (§3.3: a `SUSPENDED` step `await_event`s the verdict; the `DbVerdictSink` `emit_event`s on approval; the engine re-runs the approved step, with the `GuardCheck` handler ALLOWing via `hitl_verdict=APPROVED` so the re-run doesn't re-BLOCK; `utc_04_01` green). Still deferred: Task 4.1's cross-host SSH transport (Phase 2) and Task 4.4's `target_sha` enforcement / §1.2 race detection (column now populated by dispatch, but mismatch rejection lands with Phase 2's remote reports). `janus/` (zero `todo!`/`unimplemented!` stubs), `Makefile`, `configs/`, `blueprints/`, `workflows/`, `bin/`, and `.github/workflows/ci.yml` (CI is green). The four binaries - `janus`, `janus-daemon`, `herdr-janus`, `janush` - all exist under `janus/src/bin/`.
