...
| **Status** | ✅ Implemented — 0.5.1 (Amends ADR-021 & ADR-029). *Note: Refactored in 0.7.0 — `pipeline.rs` renamed to `src/workflow/dag.rs`; legacy `[pipeline]` parsing removed.* |
...
| **Status** | 🔄 Phase 2a Implemented (Worktree + Isolated Tmux) / Phase 2b Spec'd Only (Unprivileged OS User) — 0.7.0 (Amends ADR-001, ADR-007, ADR-031). |
