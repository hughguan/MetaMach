...
- **Implementation:**
    ...
    - **Per-blueprint DB schema:** migration `002_blueprint.sql` defines `absurd_tasks`, `absurd_steps` (`exit_code`, `started_at`, `result_cache`/`stdout_tail`) and `003_target_sha.sql` adds the `target_sha VARCHAR(64)` column; both are applied against `metamach_blueprint_<name>` on `janus onboard` (M4 Task 4.3). Cross-DB FKs to `blueprints(id)` are **dropped** (Postgres cannot enforce them across databases); `blueprint_name` is carried as a routing key, not a FK.
...
- **Implementation:**
    ...
    - **Multi-DB tenant registration:** `CREATE DATABASE metamach_blueprint_<name>` (catch SQLSTATE 42P04 => idempotent success; standard PG has no `CREATE DATABASE IF NOT EXISTS`), run `002_blueprint.sql` and `003_target_sha.sql` against it, and `INSERT ... ON CONFLICT (name) DO UPDATE` a `status='ACTIVE'` row in the **global catalog** `blueprints` table. Because `CREATE DATABASE` cannot run inside a transaction, wrap post-create steps in compensation: on any failure after the DB is created, `DROP DATABASE` to avoid a half-activated state.
