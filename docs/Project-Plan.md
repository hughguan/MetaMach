...
- **Description:** Implement `janus init` per 0.3.0 §1.4 (Multi-DB baseline), symmetric with Offboard.
- **Implementation:**
...
- **UAT:** On a clean workshop, `janus init` in joyrobots directory => `SELECT datname FROM pg_database WHERE datname='metamach_blueprint_joyrobots'` returns a row; the catalog `blueprints` table has one `ACTIVE` row; the Popup menu shows it; repeated Onboard is idempotent (42P04 caught); re-Onboard of an Offboarded blueprint recycles `production_report.md`.
