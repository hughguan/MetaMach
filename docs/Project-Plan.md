...
> **Timebox:** ~4 weeks
> **Status (2026-07-21):** Partial. Task 4.2 (Offboard) + Task 4.3 (Onboard) are implemented and tested. Task 4.1's **SQLite Log Replay is implemented** (`replay_fallback` on PG recovery), the **workflow-execution engine is implemented** (Phase 0a absurd adapter + Phase 0b `workflow::run_workflow` engine, `Request::Dispatch` Contract 3.11, `tmux_alive` wired, `utc_03_01b` green), and **cold-start re-exec + the retry loop are implemented** (Phase 1: `run_workflow` is a retry-claim loop that re-claims absurd's retry runs + resumes from the last `COMPLETED` checkpoint; `coldstart::reconcile` spawns `run_workflow` per non-terminal task instead of just logging; `max_attempts: 3`; `utc_03_03` green) - a blueprint's workflow dispatches onto absurd's pull-mode queue, drives real tmux steps under `janush` (exit-code capture, per-step checkpoints, lease renewal), retries transient failures in-session, and resumes after a daemon crash. Still **deferred**: Task 4.1's cross-host SSH transport (Phase 2), the HITL `await_event`/`emit_event` resume-loop wiring (a follow-on that consumes the same `run_workflow` resume path), and Task 4.4 (`target_sha` *enforcement* / §1.2 race detection - the column is populated by dispatch, but mismatch rejection lands with Phase 2's remote reports). These land in a future 0.5.0 arc.
