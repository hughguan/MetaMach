...
| **UTC-E2E-02** | `software-development-example` — spec2software pipeline | Validate a 3-agent pipeline implementing software from specs produced by req2spec, with the same ARCHITECT/BUILDER/TESTER cross-review and commit-push cycle | PG + tmux healthy; req2spec pipeline must have completed (specs exist in repo) | Dispatch `spec2software` pipeline against the `software-dev` blueprint. Steps follow the same pattern: BUILDER implements from `feature-spec.md`, ARCHITECT cross-reviews, TESTER validates, ARCHITECT signs off, TESTER commits. | All steps COMPLETED. Implementation matches spec. Cross-review loop resolves. Git commit exists. | **Blocker** |
