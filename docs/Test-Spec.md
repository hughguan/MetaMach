...
| **UTC-05-04** | Blueprint Onboard & Tenant Registration | Validate `janus onboard` registers tenant and makes product instantly dispatchable; operation is idempotent | Clean workshop with zero product lines; `.janus/blueprint.toml` (name `joyrobots`) in place | 1. Execute `janus onboard --blueprint joyrobots`. 2. Immediately consecutively repeat the same command once. | 1. Onboard calls `CREATE DATABASE metamach_blueprint_joyrobots` on the native PG instance. 2. `blueprints` table gains one `status='ACTIVE'` row; Popup dispatch menu instantly shows `joyrobots`. 3. Repeated execution produces no second row (`ON CONFLICT` idempotent); menu has no duplicate entries. | **Blocker** |
