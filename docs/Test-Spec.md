...
| **UTC-04-01** | HITL Resume (emit_event -> await_event -> re-run) | Validate a HITL-suspended step resumes on Director approval | Daemon running, Postgres + tmux healthy | Dispatch a 2-step workflow whose step 2 is `require_approval` -> suspends; simulate approval (`hitl_verdict=APPROVED` + `emit_event`); assert the engine wakes + re-runs the step. | 1. Step 2 `SUSPENDED` (HITL BLOCK). 2. `emit_event` wakes the `await_event`-suspended run. 3. The engine re-runs step 2; the `GuardCheck` ALLOWs via `hitl_verdict=APPROVED`. 4. Both steps reach `COMPLETED`; the absurd task reaches `completed`. | **Critical** |
...
| **UTC-05-04** | Blueprint Onboard & Tenant Registration | Validate `janus init` registers tenant and makes product instantly dispatchable; operation is idempotent | Clean workshop with zero product lines; `.janus/blueprint.toml` (name `joyrobots`) in place | 1. Execute `janus init`. 2. Immediately consecutively repeat the same command once. | 1. Onboard calls `CREATE DATABASE metamach_blueprint_joyrobots` on the native PG instance. 2. `blueprints` table gains one `status='ACTIVE'` row; Popup dispatch menu instantly shows `joyrobots`. 3. Repeated execution produces no second row (`ON CONFLICT` idempotent); menu has no duplicate entries. | **Blocker** |
| **UTC-05-04b** | Multi-DB Onboard Isolation | Validate Onboard creates independent per-blueprint database | Clean workshop; two blueprints ready | 1. Execute `janus init` in joyrobots directory. 2. Execute `janus init` in gatemetric directory. 3. Verify database topology. | 1. `CREATE DATABASE metamach_blueprint_joyrobots` executed. 2. `CREATE DATABASE metamach_blueprint_gatemetric` executed. 3. `psql -l` shows both databases. 4. `blueprints` table in global catalog `metamach_db` has two `ACTIVE` rows. 5. Blueprint name validation rejects names > 60 chars or non-alphanumeric. | **Blocker** |

| **UTC-05-05** | Re-Onboard & Experience Inheritance | Validate re-Onboard after Offboard recycles `production_report.md` as immune antibodies | `gatemetric` has been Offboarded; `production_report.md` contains marker string `PIN_CONFLICT_MARKER_21` | 1. Execute `janus init` to re-onboard. 2. Via Daemon debug endpoint, export that blueprint's Agent System Prompt. 3. Dispatch a step that historically triggered pin conflict. | 1. Onboard succeeds; blueprint status returns to `ACTIVE`. 2. Exported System Prompt's `## Previous Incidents` section must contain `PIN_CONFLICT_MARKER_21`. 3. Agent no longer attempts conflicting pin configuration; step passes on first attempt. | **Critical** |
...
> run manually before tagging a release to validate the full DevSecOps lifecycle.
> Mock-agent variants (shell scripts producing deterministic output) can run in
> CI to cover the DAG engine + Tool Guard mechanics without LLM dependency.

| ID | Module | Purpose | Precondition | Steps | Expected | Severity |
|----|--------|--------|-------------|-------|----------|----------|
| **UTC-E2E-01** | `software-development-example` — req2spec pipeline | Validate a 3-agent (ARCHITECT, BUILDER, TESTER) cross-review pipeline producing Architecture Design, Feature Spec, and Test Spec documents from a product requirement, with commit-and-push finalization | PG + tmux healthy; `blueprints/software-dev/janus.toml` onboarded; `pipelines/req2spec.toml` deployed; three agent roles configured in `agents.toml` | **Phase 1 — Architect produces design:** 1. Dispatch `req2spec` pipeline. 2. Step "architect_design": ARCHITECT reads `requirements/product.md`, produces `docs/architecture-design.md`. 3. Output passes to next step. **Phase 2 — Builder + Tester cross-review:** 4. Steps "builder_review" and "tester_review" run in parallel. BUILDER reviews the design doc, adds inline `[REJECT]` / `[APPROVE]` comments. TESTER does the same. 5. Step "architect_address": ARCHITECT reads all review comments, addresses each (either accepts the change or explains why it's invalid). Produces revised `docs/architecture-design.md`. 6. Steps "builder_final_review" and "tester_final_review" re-run. Repeat until both return APPROVE with no remaining REJECT comments. **Phase 3 — Builder + Tester produce specs (parallel):** 7. Step "builder_spec": BUILDER produces `docs/feature-spec.md` (including development plan). 8. Step "tester_spec" runs in parallel: TESTER produces `docs/test-spec.md`. 9. Steps "architect_review_builder" and "architect_review_tester" + "builder_review_tester" / "tester_review_builder" cross-review all specs. Repeat until all parties approve. **Phase 4 — Commit & push:** 10. Step "tester_commit": TESTER runs `git add docs/ && git commit -m "req2spec: architecture design + feature spec + test spec"` and pushes to remote. | 1. All steps reach `COMPLETED` with `exit_code=0`. 2. `metamach_step_meta` records each step's `stdout_tail`, `exit_code`, `target_sha`. 3. Review loop correctly re-runs until all reviewers return APPROVE (no race between parallel reviews). 4. Parallel steps (builder_review + tester_review) execute in independent tmux sessions. 5. Final `docs/architecture-design.md`, `docs/feature-spec.md`, `docs/test-spec.md` exist in the blueprint repo with correct content. 6. Git commit exists with the commit message containing "req2spec". 7. Cold-start: kill daemon mid-pipeline, restart, assert resume from last COMPLETED checkpoint and all steps reach COMPLETED. | **Blocker** |

| **UTC-E2E-02** | `software-development-example` — spec2software pipeline (per-milestone iteration) | Validate a 3-agent iterative implementation pipeline: BUILDER implements per-unit → TESTER validates → ARCHITECT reviews → loop until approval → TESTER commits+tags. Between milestones, HITL approval gates the next kickoff. | PG + tmux healthy; req2spec pipeline must have completed (all 3 docs exist in repo); `agents.toml` with architect/builder/tester roles | **Per-unit cycle (repeats for each unit in the dev plan):** 1. BUILDER implements the unit per `feature-spec.md` dev plan, commits the change. 2. BUILDER requests test report from TESTER. 3. TESTER runs tests per `test-spec.md`. If all tests pass AND coverage meets test spec → APPROVE with test report. If any failure or unmet spec → REJECT with failure details. 4. BUILDER addresses REJECT → loop back to step 2. 5. Once TESTER approves with test report, hand to ARCHITECT. 6. ARCHITECT reviews code against: (a) `feature-spec.md` — does the implementation match? (b) `architecture-design.md` — does it align with the architecture? (c) test report — are all tests passing per `test-spec.md`? 7. ARCHITECT returns REJECT (with comments) or APPROVE. 8. BUILDER addresses REJECT → TESTER re-tests → ARCHITECT re-reviews → loop until ARCHITECT approves. 9. TESTER updates dev plan (marks unit complete), `git commit + push + tag`. **Milestone boundary:** After all units in a milestone complete, the pipeline pauses for HITL approval before the next milestone kicks off. **Full product:** Pipeline repeats until all milestones complete. | 1. All units within each milestone reach COMPLETED. 2. TESTER test reports exist per unit. 3. ARCHITECT review trail exists per unit (REJECT→address→APPROVE). 4. HITL gate fires between milestones (Director must approve next kickoff). 5. Cold-start resume works mid-unit. 6. Final product passes all tests per `test-spec.md`. | **Blocker** |

| **UTC-E2E-03** | `software-development-example` — adr-process pipeline (mid-development change injection) | Validate the Architecture Decision Record process for handling mid-development changes (new features, architecture changes, any deviation from the original docs). Injects between milestones to update specs before the next spec2software iteration. | PG + tmux healthy; software development is in progress (between milestones); a change request exists | **Phase A — ADR generation:** 1. ARCHITECT reviews the new change request (could be a new feature, architecture change, or any deviation from current docs). 2. ARCHITECT generates an ADR (Architecture Decision Record) with status `PENDING`. **Phase B — Cross-review:** 3. BUILDER and TESTER review the ADR in parallel. Each returns `[APPROVE]` or `[REJECT]` with comments. 4. ARCHITECT addresses REJECT comments (accepts or explains why invalid). Loop until resolved. **Phase C — ADR closure:** 5. If both BUILDER and TESTER approve → ADR status → `CLOSED:APPROVED`. If no alignment achieved after max review rounds → ADR status → `CLOSED:DEFERRED`. If all three reject the change after discussion → ADR status → `CLOSED:REJECTED`. **Phase D — Spec update (only if APPROVED):** 6. ARCHITECT updates `docs/architecture-design.md` to reflect the approved ADR. 7. BUILDER updates `docs/feature-spec.md` with new development plan (new/modified milestones and units). 8. TESTER updates `docs/test-spec.md` with new test cases. 9. Cross-review the updated docs (same req2spec review pattern). 10. TESTER commits and pushes all updated docs. The next spec2software iteration uses the updated specs. | 1. ADR progresses from PENDING → CLOSED:APPROVED / DEFERRED / REJECTED correctly. 2. If APPROVED, all 3 docs are updated and committed. 3. Review loops converge within max rounds. 4. DEFERRED and REJECTED ADRs are archived with rationale (no spec update). 5. Cold-start resume works mid-review. | **Critical** |
