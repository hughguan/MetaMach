...
| **Idempotency Keys** (spawn-time dedup) | Prevent duplicate task spawning | Schema supports it (`idempotency_key` column exists); **not passed from Rust** | MetaMach dispatches are user-initiated (CLI `janus dispatch` or TUI), not automated schedulers. Duplicate protection isn't needed when a human is pressing the button. |
