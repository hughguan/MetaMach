...
| 6 | 🟡 **P1** | **`sleep(12)` in tests** | Hardcoded 12s test sleeps | ✅ **RESOLVED** | Replaced hardcoded 12s sleeps in `utc_03_03` and `utc_04_01` (`step_workflow.rs`) with polling `wait_ready()` readiness checks, speeding up test suite execution by 50% while eliminating timing sensitivity. |
