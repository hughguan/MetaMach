...
> **Codebase size**: 11,212 LOC (source) + 3,138 LOC (tests) | **Test count**: 171 (154 `#[test]` functions + parameterized expansions) | **ADRs**: 30
...
> 3. **CI & Testing**: macOS CI added, `cargo audit` active, `make test/lint/ci` targets integrated, 171 tests green across Linux & macOS.
> 4. **Formal Rejections**: Hardcoded test sleeps (PG crash recovery reconnect buffer), archival `Project-Plan.md` title, pool sizes (YAGNI/PG client bounds), coverage tools (`ptrace` PTY interference), and Tool Guard doc redundancy rejected with cause.
...
| Testing | 7.5/10 | **9.0/10** | ↑ 1.5 | macOS CI; `cargo audit`; concurrent isolation tests; 171 green tests; PG crash recovery buffers |
...
| 6 | 🟡 **P1** | **`sleep(12)` in tests** | Hardcoded 12s test sleeps | ❌ **REJECTED (WITH CAUSE)** | The 12s sleeps in `utc_03_03` and `utc_04_01` are necessary PG socket/pool reconnection buffers following an ungraceful `SIGKILL` of the daemon process. Simple UDS `Ping` polling checks socket binding, not PG backend reconnection after a crash. |
...
| 15 | 🟢 **P3** | **Coverage Reporting** | No `tarpaulin`/`grcov` | ❌ **REJECTED (WITH CAUSE)** | `ptrace` instrumentation in coverage tools interferes with real `tmux` PTY signal handling in integration tests and adds 3+ minutes of CI overhead for minimal gain over 171 integration tests. |
...
- **171 integration and unit tests**, green across both Linux and macOS CI pipelines.
