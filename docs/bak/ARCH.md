...
> **EN:** MetaMach 0.3.0 Architecture — Janus Daemon & Distributed Durable Sessions: a silicon-grade industrial production machine.
...
**MetaMach 0.3.0** 彻底颠覆了这种脆弱的拓扑。它采用 **“守护进程为主体，影子插件为外壳”** 的高凝聚松耦合架构，将整套系统解耦为 **生产要素（Agent Pool）**、**工艺流水线（Workflows）** 与 **产品蓝图（Blueprints）**：
...
> **设计决策 — Git 保持为纯粹代码仓库：** Offboard **绝不**执行任何 `git commit --amend` 或其他破坏性 Git 历史改写动作。审计轨迹、Agent 签名、测试指标的单一权威源是 Absurd Postgres。Git 仅作为代码版本控制系统；共识签名与过程元数据绝不嵌入 Git 提交消息。`production_report.md` 是唯一提交至 Git 的知识产物，且以普通增量提交（非 amend）完成。
        

## 3. 系统架构拓扑 (System Architecture)

MetaMach 0.3.0 实行“大脑独立监控、影子客户端透传、物理会话挂接、数据逻辑多租户”的工业化隔离方案：

- **Control Plane (控制层)：**
    
    - **`janus-daemon` (常驻进程)：** 负责核心逻辑调度，维持与 Absurd Postgres 的长连接，监听外部 Teams/TG 异步消息。同时对外暴露 `progress` 查询原语：聚合 `absurd_tasks` JOIN `absurd_steps` 的实时状态与 Tether 物理 Session 存活信号，作为工作流进度大盘的唯一权威数据源。
...
- **Physical Execution Plane (物理执行层)：**
    ...
    - **`janus::tether` (物理引擎)：** `janus-daemon` 内部的原生 Rust 模块，直接管理 tmux `remain-on-exit` 会话与跨主机 SSH 传输。原外部 `herdr-tether` 插件已于 0.3.0 内置化。
        
- **Persistence Plane (持久化与知识层)：**
    
    - **Absurd Postgres (Absurd DB)：** One PG, Multi-DB 拓扑。单一物理 Postgres 实例（原生，无 Docker）承载独立逻辑数据库（Onboard 时 `CREATE DATABASE metamach_blueprint_<name>`）。每个蓝图完全隔离，拥有独立连接池，消除跨蓝图锁竞争。数据持久化于 `~/.metamach/db/`。
...
> **CLI 与二进制架构（统一入口 + 专职二进制）**：系统以统一 `janus` CLI 作为厂长与管理面的单一入口，子命令分两类：(1) **生命周期/查询子命令**--`janus onboard`、`janus offboard`、`janus status`，均为轻量客户端，经 UDS 与常驻 `janus-daemon` 通信（**Daemon 必须在运行**；`janus daemon` 子命令显式拉起它）；(2) 底层专职二进制--`janus-daemon`（常驻大脑）、`herdr-janus`（影子客户端，由 Herdr 加载）、`janus-sh`（代理 Shell，由 Tether 注入为 `SHELL`）；（4）`janus::tether` — 物理执行引擎，已于 0.3.0 内置化为 `janus-daemon` 原生模块。即 `janus offboard` 等价于“客户端经 UDS 向 Daemon 发起 offboard 指令”，而非独立直连数据库。所有 Tether 操作均由 `janus-daemon` 内部管理；`herdr-tether` CLI 二进制不再作为独立外部依赖。
...
sequenceDiagram
    ...
    participant Absurd as Absurd Postgres (状态库)
    participant Guard as Tool Guard (janus-sh)
    participant Tether as janus::tether (内置)
...
> 该查询为**只读旁路**：不占用工作流执行的事务通道，不影响在跑工位，仅读取 Absurd Postgres 中的权威状态。无 TUI 环境下，`janus status` CLI 走同一 `progress` 原语输出纯文本/JSON 快照。
...
> ⚠️ **关于外部依赖与动态配置的说明：**
> - `absurd`、`openwiki` 均为独立外部仓库，不在本 monorepo 中编译。`make bootstrap` 负责拉取/构建/链接这些依赖。`herdr-tether` 已于 **0.3.0 废弃** — 其 tmux 会话引擎已内置化为 `janus::tether`。
> - 运行时 Mutable 配置（如 `agents.toml`）必须被软链接放置在 根据官方 Spec 规范，运行时的 Mutable 配置（如 `agents.toml`）必须被软链接放置在 **`${HERDR_PLUGIN_CONFIG_DIR}`**（即 `~/.config/herdr/plugins/metamach.janus`）中； 所有的事务日志、缓存的 SQLite 及临时 Socket 文件必须存放在 **`${HERDR_PLUGIN_STATE_DIR}`**（即 `~/.local/state/herdr/plugins/metamach.janus`）下。这样可以彻底确保 GitHub 在更新插件源码时，**绝不意外抹除任何本地财务或开发状态数据**。**数据库持久化** 使用 `~/.metamach/db/` — 独立于 Herdr 插件生命周期的全局目录，确保 PG 数据在插件升级与断电重启后完整保留。
...
5. **版本对账 — Git SHA 乐观锁自愈：** 为防止慢速远程真机测试报告（可能耗时数分钟）在回传时盲目覆盖本地已演进的最新代码状态，系统在 `absurd_steps` 级实行 SHA-1 乐观锁对账。每次 Step 派发时将当前 `HEAD` SHA 固化入库。测试结果返回时，Daemon 校验 `current_sha == target_sha`。若本地 HEAD 已改变（`UPDATE … WHERE target_sha = $4` 返回零行），Daemon 触发 `CONCURRENCY_RACE_ALERT`，丢弃过期报告，自动为当前 HEAD 重新排产测试，物理上彻底消除版本错配。
